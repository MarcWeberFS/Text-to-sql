...
public class BenchmarkResultService {
        ...
        public List<Map<String, Object>> getBenchmarkResults(int runNumber) {
                return queryService.executeQuery(
                        "select id, is_correct, llm, human_correction, benchmark_case_id from benchmark_results where run_number = "
                                + runNumber + " order by benchmark_case_id, llm");
        }

        public List<Map<String, Object>> getBenchmarkCase(int id) {
                return responseSanitizer
                        .sanitizeResult(queryService.executeQuery("select * from benchmark_cases where id = " + id));
        }

        public List<Map<String, Object>> getBenchmarkCaseResult(int id, int runNumber) {
                return responseSanitizer.sanitizeResult(queryService.executeQuery(
                        "select * from benchmark_results where run_number = " + runNumber + " and benchmark_case_id = " + id));
        }
        ...
        Map<String, Object> original = queryService
                .executeQuery("""
                SELECT
        ...
        Map<String, Object> stats = new java.util.HashMap<>(original);
