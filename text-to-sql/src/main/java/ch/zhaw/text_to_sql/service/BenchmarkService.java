    ...
    public void runAllBenchmarks(boolean user_feedback_loop, boolean syntax_feedback_loop) {
        ...
        for (String llm : llms) {
            for (BenchmarkCase testCase : benchmarkCases) {
                ...
                if (alreadyExists > 0) {
                    System.out.printf("Skipping benchmark case ID %d for LLM '%s' (already stored)%n", testCase.getId(), llm);
                    continue;
                }
                ...
                try {
                    ...
                    System.err.printf("Error during benchmark ID %d for LLM %s: %s%n", testCase.getId(), llm, e.getMessage());
                }
            }
        }
    }
